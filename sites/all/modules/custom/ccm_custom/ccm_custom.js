...
Drupal.openlayers.addBehavior('openlayers_behavior_zoomtolayer', function (data, options) {
  ...
  var layers_tmp = layers;
  for (var i in layers_tmp) {
    if (layers_tmp[i].layer_handler == 'geofile_hybrid') {
      for (var j in layers_tmp[i].layers) {
        layers[layers.length] = layers_tmp[i].layers[j];
      }
    }
  }
